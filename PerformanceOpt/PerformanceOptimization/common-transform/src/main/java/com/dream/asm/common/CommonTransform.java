...
import com.dream.asm.common.weaver.ClassLoaderHelper;
...
public class CommonTransform extends Transform {
    ...
    public void transform(Context context,
        ...
        logger.warn(getName() + " isIncremental = " + isIncremental + ", runVariant = "
                + runVariant + ", emptyRun = " + emptyRun + ", inDuplcatedClassSafeMode = "
                + inDuplcatedClassSafeMode());
        long startTime = System.currentTimeMillis();
        if (!isIncremental) {
            outputProvider.deleteAll();
        }
        URLClassLoader urlClassLoader = ClassLoaderHelper
        ...
        for (TransformInput input : inputs) {
            for (JarInput jarInput : input.getJarInputs()) {
                ...
                if (isIncremental && !emptyRun) {
                    ...
                    if (inDuplcatedClassSafeMode() & !isIncremental
                            && !flagForCleanDexBuilderFolder) {
            ...
            for (DirectoryInput directoryInput : input.getDirectoryInputs()) {
                ...
                if (isIncremental && !emptyRun) {
                    ...
                    for (Map.Entry<File, Status> changedFile : fileStatusMap.entrySet()) {
                        ...
                        String destFilePath = inputFile.getAbsolutePath()
                                .replace(srcDirPath, destDirPath);
